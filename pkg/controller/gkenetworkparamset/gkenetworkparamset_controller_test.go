...
func TestGKENetworkParamSetValidations(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedCondition: metav1.Condition{
				...
				Reason: "GNPConfigInvalidArgument",
			},
		},
		{
		...
		{
			name: "GNP with NetworkAttachment - on default network",
			paramSet: &networkv1.GKENetworkParamSet{
				ObjectMeta: metav1.ObjectMeta{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			defSubnetKey := meta.RegionalKey(defSubnet.Name, testVals.clusterValues.Region)
			err = testVals.cloud.Compute().Subnetworks().Insert(ctx, defSubnetKey, defSubnet)
			if err != nil {
				t.Error(err)
			}
...
func TestCrossValidateNetworkAndGnp(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "L3NetworkType with missing both PodIPv4Ranges and NetworkAttachment",
			network: &networkv1.Network{
				ObjectMeta: metav1.ObjectMeta{
					Name: networkName,
				},
